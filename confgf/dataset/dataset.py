...
def group_same_size(
    ...
    for i in range(len(data_list)):
        data = Data.from_dict(data_list[i].__dict__)
...
def preprocess_iso17_dataset(base_path):
    ...
    print(all_train[0])
    #TODO: make iso 17 into right shape
...
def get_GEOM_testset(base_path, dataset_name, block, tot_mol_size=200, seed=None, confmin=50, confmax=500):
    ...
    block_smiles = defaultdict(int) #no key error
    for block_ in block:
...
class GEOMDataset(Dataset):
    ...
    def __init__(self, data=None, transform=None, batch_size=16):
        super().__init__()
        self.batch_size = batch_size
        self.data = data
        self.grouped_data = group_same_size(data, transform=transform)
    ...
    def __getitem__(self, idx):
        ...
        data = self.batched_data[idx].clone()
        #pos_center = data.pos.mean(dim=0) #TODO: add pos center when checking ClofNet
...
class GEOMDataset_PackedConf(GEOMDataset):
    ...
    def __init__(self, data=None, transform=None):
        ...
        self.grouped_data = group_same_size(self.new_data)
        self.batched_data = batch_same_size(self.grouped_data, self.batch_size)
    ...
    def _pack_data_by_mol(self):
        ...
        for k, v in self._packed_data.items():
            ...
            for i in range(len(v)):
                ...
                pos = v[i].pos - pos_center #TODO: remove
                all_pos.append(pos)
